...
Blockly.WarningHandler.prototype['checkGql'] = function(block) {
  ...
  if (parentBlock.typeName === 'GraphQL') {
    // Only perform check when attached to the query argument.
    if (parentBlock.getArgInputs()['query'].connection.targetBlock() !== block) {
      return;
    }
    ...
    if (!schema.types[Blockly.GraphQLBlock.ROOT_TYPE].fields.hasOwnProperty(block.gqlName)) {
  ...
  if (block.gqlBaseType === undefined || parentBlock.gqlBaseType === undefined) {
    return false;
  }
  ...
  if (block.gqlParent === null) {
    ...
    var validType = goog.array.find(allPossibleTypes, function(type) {
      return Blockly.GraphQLBlock.traverseTypeRef(type).name === block.gqlBaseType;
    });

    // Check for type validity.
    if (!validType) {
      if (this.showWarningsToggle) block.setWarningText('This fragment is not valid in the parent object.');
      return true;
    }
  }
  ...
  // Perform checking on non-fragments.
    ...
    if (block.gqlParent !== parentBlock.gqlBaseType || !parentType.fields.hasOwnProperty(block.gqlName)) {
